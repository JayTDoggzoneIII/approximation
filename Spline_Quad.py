...
def main() -> int:
    ...
    for n in [3,10,20]:
        ...
        a_s = []
        for i in range(n-1):
            ai2, ai1, ai0 = Symbol('a%d2'%(i+1), real = True), Symbol('a%d1'%(i+1), real = True), Symbol('a%d0'%(i+1), real = True)
            a_s.append(ai2)
            a_s.append(ai1)
            a_s.append(ai0)
            l_s.append(ai2*x*x + ai1*x + ai0)
        to_solve = [l_s[0].subs(x,nodes_list[0]) - nodes[nodes_list[0]], l_s[-1].subs(x,nodes_list[-1]) - nodes[nodes_list[-1]],(l_s[-1].diff(x)).subs(x,nodes_list[-1])]
        ...
        for i in range(n-1):
            ai2, ai1, ai0 = Symbol('a%d2'%(i+1), real = True), Symbol('a%d1'%(i+1), real = True), Symbol('a%d0'%(i+1), real = True)
            l_ans.append((l_s[i].subs([(ai2,ans[ai2]), (ai1,ans[ai1]),(ai0,ans[ai0])]),nodes_list[i],nodes_list[i+1]))
        for func,start,end in l_ans:
            print("S2,1(x) = %s,  from %.6f to %.6f"%(func, start, end))
        deform = []
        for i in range(n-1):
    ...
    for n in [3,10,20]:
        ...
        for i in range(n-1):
            ...
            l_ans.append((l_s[i].subs([(ai2,ans[ai2]), (ai1,ans[ai1]),(ai0,ans[ai0])]),nodes_list[i],nodes_list[i+1]))
        for func,start,end in l_ans:
            print("Sopt_2,1(x) = %s,  from %.6f to %.6f"%(func, start, end))
        deform = []
        for i in range(n-1):
            #deform.append((l_ans[i][0],l_ans[i][1] <= x))
            deform.append((l_ans[i][0],x < l_ans[i][2]))
